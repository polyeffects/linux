...
#define PCM3168A_FORMATS ( SNDRV_PCM_FMTBIT_S24_3LE | \
				  ...
				  int clk_id, unsigned int freq, int dir)
{
	 ...
	 */
	/* printk("PCM3168a COOPS %s freq is %u", */
	/* 	__func__, freq); */
	if (freq == 0)
		return 0;
...
static void pcm3168a_update_fixup_pcm_stream(struct snd_soc_dai *dai)
{
	struct snd_soc_component *component = dai->component;
	struct pcm3168a_priv *pcm3168a = snd_soc_component_get_drvdata(component);
	u64 formats = SNDRV_PCM_FMTBIT_S24_3LE | SNDRV_PCM_FMTBIT_S24_LE | SNDRV_PCM_FORMAT_S32_LE ;
	/* unsigned int channel_max = dai->id == PCM3168A_DAI_DAC ? 8 : 6; */

	/* if (pcm3168a->io_params[dai->id].fmt == PCM3168A_FMT_RIGHT_J) { */
	/* 	/1* S16_LE is only supported in RIGHT_J mode *1/ */
	...
	if (dai->id == PCM3168A_DAI_DAC) {
		dai->driver->playback.channels_max = 8;
		dai->driver->playback.formats = formats;
	} else {
		dai->driver->capture.channels_max = 6;
...
static int pcm3168a_set_dai_fmt(struct snd_soc_dai *dai, unsigned int format)
{
	...
	/* printk("PCM3168a COOPS %s format is %u master_mode %d", */
	/* 	__func__, format, master_mode); */
				 ...
				 int slot_width)
{
	...
	/* printk("PCM3168a COOPS before set %s rx_mask is %u tx_mask  %u, slot width is %d tdm_slots %d", */
	/* 	__func__, rx_mask, tx_mask, slot_width, slots); */
	...
	/* printk("PCM3168a COOPS %s rx_mask is %u tx_mask  %u, slot width is %d tdm_slots %d", */
	/* 	__func__, rx_mask, tx_mask, slot_width, slots); */
			     ...
			     struct snd_soc_dai *dai)
{
	...
	/* printk("PCM3168a COOPS before slot %s rate is %d sysclk  %lu, ratio %u, slot width is %d", */
	/* 	__func__, rate, pcm3168a->sysclk, ratio, slot_width); */
	...
	/* printk("PCM3168a COOPS %s rate is %d sysclk  %lu, slot width is %d tdm_slots %d", */
	/* 	__func__, rate, pcm3168a->sysclk, slot_width, tdm_slots); */
	...
	reg = PCM3168A_RST_SMODE;
	mask = PCM3168A_DAC_SRDA_MASK;
	shift = PCM3168A_DAC_SRDA_SHIFT;
	regmap_update_bits(pcm3168a->regmap, reg, mask, 1 << shift);

	reg = PCM3168A_ADC_SMODE;
	mask = PCM3168A_ADC_SRAD_MASK;
	shift = PCM3168A_ADC_SRAD_SHIFT;
	regmap_update_bits(pcm3168a->regmap, reg, mask, 1 << shift);
