			       ...
			       unsigned int format, bool dac)
{
	...
	//printk("++++ set dai format, %u", format);
	...
	if (dac) {
		...
		printk("PCM3168a ++++ dac_master_mode, %d", master_mode);
	} else {
		...
		printk("PCM3168a ++++ adc_master_mode, %d", master_mode);
	}
			     ...
			     struct snd_soc_dai *dai)
{
	...
	printk("++++ PCM3168a rate, %u chan, %u ratio %u", rate, chan, ratio);
	...
	if (chan > 2) {
		...
		case PCM3168A_FMT_I2S:
			...
			//printk("++++ format is I2S TDM");
			break;
		case PCM3168A_FMT_LEFT_J:
			fmt = PCM3168A_FMT_LEFT_J_TDM;
			//printk("++++ format is LEFT_J TDM");
			break;
		default:
			    ...
			    struct snd_soc_dai *dai)
{
	...
	case PCM3168A_FMT_RIGHT_J:
		sample_min  = 16;
		channel_max =  2;
		//printk("++++ startup format is right");
		break;
	case PCM3168A_FMT_LEFT_J:
	case PCM3168A_FMT_I2S:
		sample_min  = 24;
		channel_max = channel_maxs[tx];
		//printk("++++ startup format is left / i2s");
		break;
	default:
		//printk("++++ startup format is default");
		sample_min  = 24;
		channel_max =  2;
	}
