				  ...
				  unsigned int slot_width)
{
	...
	if (!sun4i_i2s_oversample_is_valid(oversample_rate)) {
		dev_err(dai->dev, "Unsupported oversample rate: %d rate %d mclk %d\n",
			oversample_rate, rate, i2s->mclk_freq);
		return -EINVAL;
	}
...
static int sun8i_i2s_get_sr(unsigned int width)
{
	/* printk("COOPS %s ## sr width is %u", */
	/* 	__func__, width); */
	width = 32;
	switch (width) {
	case 8:
...
{
	/* printk("COOPS %s ## slot width is %u", */
	/* 	__func__, width); */
	width = 32;
	switch (width) {
	case 8:
				  ...
				  unsigned int slot_width)
{
	...
	/* channels = 8; */
	/* printk("COOPS %s channels is %d, slots  %d, slot width is %d", */
	/* 	__func__, channels, slots, slot_width); */

	/* Map the channels for playback and capture */
	regmap_write(i2s->regmap, SUN8I_I2S_TX_CHAN_MAP_REG, 0x76543210);
	regmap_write(i2s->regmap, SUN8I_I2S_RX_CHAN_MAP_REG, 0x75643120);
	/* regmap_write(i2s->regmap, SUN8I_I2S_TX_CHAN_MAP_REG, 0x01234567); */
	/* regmap_write(i2s->regmap, SUN8I_I2S_RX_CHAN_MAP_REG, 0x01234567); */

	/* Configure the channels */
	regmap_update_bits(i2s->regmap, SUN8I_I2S_TX_CHAN_SEL_REG,
			   SUN4I_I2S_CHAN_SEL_MASK,
			   SUN4I_I2S_CHAN_SEL(8));
	...
	regmap_update_bits(i2s->regmap, SUN8I_I2S_CHAN_CFG_REG,
			   ...
			   SUN8I_I2S_CHAN_CFG_TX_SLOT_NUM(8));
	regmap_update_bits(i2s->regmap, SUN8I_I2S_CHAN_CFG_REG,
			   SUN8I_I2S_CHAN_CFG_RX_SLOT_NUM_MASK,
			   SUN8I_I2S_CHAN_CFG_RX_SLOT_NUM(6));
	...
	/* printk("COOPS %s set chan_en_mask, channels is %d, slots  %d, lrck_period is %d", */
	/* 	__func__, channels, slots, lrck_period); */

	regmap_update_bits(i2s->regmap, SUN8I_I2S_TX_CHAN_SEL_REG,
			   SUN8I_I2S_TX_CHAN_EN_MASK,
			   SUN8I_I2S_TX_CHAN_EN(8));
			       ...
			       struct snd_soc_dai *dai)
{
	...
	/* printk("COOPS %s ------ set chan cfg  ret is %d", */
	/* 	__func__, ret); */
	if (ret < 0) {
	...
	default:
		...
		/* printk("COOPS %s ------ physical param sample width is set %d", */
		/* 	__func__, params_physical_width(params)); */
		return -EINVAL;
	}
	/* printk("COOPS %s -----  dma width is %u", */
	/* 	__func__, width); */
				 ...
				 unsigned int fmt)
{
	...
	case SND_SOC_DAIFMT_CBS_CFS:
		...
		/* printk("COOPS %s setting sunxi BCLK_MASTER\n", __func__); */
		break;

	case SND_SOC_DAIFMT_CBM_CFM:
		/* BCLK and LRCLK slave */
		val = 0;
		/* printk("COOPS %s setting sunxi BCLK_slave\n", __func__); */
		break;
	...
	/* printk("COOPS %s setting sunxi sign extension\n", __func__); */
...
#define SUN4I_FORMATS	( SNDRV_PCM_FMTBIT_S24_LE | \
