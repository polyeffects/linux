				  ...
				  unsigned int word_size)
{
	...
	if (i2s->bit_clk_master) {
		 // printk("#######***** ERROR entering bit_clk_master");
		switch (rate) {
		case 176400:
		case 88200:
		case 44100:
	...
	/* Set sync period */
    int lrck;
    /* lrck = 128; // #might be 255 XXX macro removes 1. */
    lrck = 64; // #might be 255 XXX macro removes 1. in master mode, mclk 12288
			       ...
			       struct snd_soc_dai *dai)
{
	...
	//channels =  8; 
    channels = params_channels(params); // override to 8?
    // // printk("#+# channels is %d", channels);
    /* Enable the required output line XXX */
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
        ...
        // FF7
        // // printk("#+# writing txchansel");
        regmap_field_write(i2s->field_txchansel, channels-1);

		regmap_field_write(i2s->field_txchanmap, 0x61407352);
        ...
        // reg 54
		/* regmap_field_write(i2s->field_rxchansel, SUN4I_I2S_CHAN_SEL(8)); */
        regmap_field_write(i2s->field_rxchansel, channels-1);
		/* regmap_field_write(i2s->field_rxchansel, 0x7); */
		/* regmap_field_write(i2s->field_rxchansel, 0x7); */
        // 31
		/* regmap_field_write(i2s->field_rxchanmap, 0x61407352); */
		/* regmap_field_write(i2s->field_rxchanmap, 0x61407352); */
		regmap_field_write(i2s->field_rxchanmap, 0x76543210);
	}
	...
	/*  // printk("sr is %d wss is %d", sr, wss); */
	/* regmap_field_write(i2s->field_fmt_wss, 0x7); */
	/* regmap_field_write(i2s->field_fmt_sr, 0x5); */
...
static int sun4i_i2s_set_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	/*  // printk("sun4i_i2s entering set fmt\n"); */
    ...
    // // printk("#!# lrclk_polarity is inverted %d", lrclk_polarity == SUN4I_I2S_FMT0_POLARITY_INVERTED);
    lrclk_polarity = SUN4I_I2S_FMT0_POLARITY_INVERTED;
    // // printk("#!# forcing lrclk_polarity is inverted %d", lrclk_polarity == SUN4I_I2S_FMT0_POLARITY_INVERTED);
	...
	if (i2s->variant->has_slave_select_bit) {
		...
		case SND_SOC_DAIFMT_CBS_CFS:
			...
			/*  // printk("slave bit COOPS SUN4I_I2S_CTRL_REG IS MASTER\n"); */
		...
		case SND_SOC_DAIFMT_CBM_CFM:
			...
			/*  // printk("slave bit COOPS SUN4I_I2S_CTRL_REG IS SLAVE\n"); */
			break;
		default:
		...
		case SND_SOC_DAIFMT_CBS_CFS:
			/* BCLK and LRCLK master */
			/*  // printk("COOPS SUN4I_I2S_CTRL_REG IS MASTER\n"); */
			/*  // printk("forcing slave\n"); */
			// XXX Force slave
	...
	/*  // printk(" in SET FMT %s fmt value is 0x%x. Setting the following registers.\n", __func__, fmt); */
...
static int sun4i_i2s_probe(struct platform_device *pdev)
{
    ...
    // default rate XXX
    // // printk ("------ in I2S forcing MCLK on in probe");
	// disable for master mode
    /* regmap_write(i2s->regmap, SUN4I_I2S_CLK_DIV_REG, */
    /*          SUN4I_I2S_CLK_DIV_BCLK(16) | */
    /*          SUN4I_I2S_CLK_DIV_MCLK(4)); // not final values */
